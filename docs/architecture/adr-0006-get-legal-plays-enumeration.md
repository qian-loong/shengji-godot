...
- **性能：单次 `get_legal_plays()` 远低于 16.6ms 帧预算**——2D 卡牌、AI 决策非每帧调用，但 25 张手牌
  的跟牌枚举若暴力组合会爆炸（C(25,k) 量级），必须靠"域内优先 + 结构约束"剪枝。
...
- **性能**：25 张手牌最坏情形单次枚举 < 5ms（宽松上界，实测应远低于此）。
...
|--------|--------|---------------|--------|
| CPU (单次 get_legal_plays, 25 张最坏) | N/A（未实现） | < 5ms | 16.6ms 帧预算，AI 决策非每帧 |
| Memory | N/A | 每次枚举 ≤ 数百候选 × Array 引用，KB 量级瞬时 | 256MB 移动端上限 |
| Load Time | 0 | 无影响 | — |
| Network | N/A | 不适用（本地单机） | — |

> 域内分治使实际枚举空间远小于 C(25,k)：每域独立枚举，各域牌数 ≤ 13（副）/更少；
> 跟牌枚举受结构约束进一步剪枝。2D 卡牌决策频率低，5ms 是宽松上界。
