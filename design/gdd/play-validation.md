...
```
PlayValidator.challenge_dump(
    dump_cards: Card[],
    other_hands: Card[][],      # 其他三家手牌
    trump_suit: int,
    current_rank: int,
    rule_config: RuleConfig,
) → { ok: bool, fallback_card: Card, reason: String }
```

1. `allow_dump` 必须为 true（由 `validate_lead` 先行拦截）
2. 所有牌同属一个花色域（同上）
3. 拆解为最大牌型优先：拖拉机 > 对子 > 单张
4. 每个组成部分在其他 3 人手中该花色域内无**更大**的同类型牌（相等不构成挑战）
5. 失败时返回 `ok=false` 与 `fallback_card`（选中牌中最小的一张单牌），
   由会话层降级重走首出校验，其余牌收回

> `trump_suit` / `current_rank` 必须显式传入——它们是每局的运行时状态，
> 不在 `RuleConfig` 里。
>
> **`other_hands` 是引擎裁决用的全局视野**：仅在 `SessionController.submit_play`
> 中调用，AI 决策路径（`AIPlayer.decide_play`）不得触碰本方法，否则等同开天眼。
...
| **Dump** | 按甩牌的拆解结构逐分量匹配：所需对子数 = 各分量对子数之和（拖拉机计 pair_count，对子计 1，单张计 0）。手中对子不足时按 `min(手中对子数, 所需对子数)` 要求。**对上对子后，剩余牌位任选** |

> **这条约束是平衡杠杆，不是格式检查。**
>
> 不许拆对意味着跟牌方被迫交出更多分：首出对 K 时手握对 10，
> 严格模式下必须送 20 分，宽松模式可拆开只送 10 分（且此后手中无对，
> 不再受该约束）。因此它直接决定"首出对子/甩牌能不能逼对手交分"。
>
> 首出为甩牌时**同样生效**——否则甩牌反而比出对子更逼不出分，攻击力倒挂。
...
| ~~Q3~~ | ~~跟甩牌时如果只能部分匹配结构，剩余部分的选择是否需要约束（必须出最大？还是任意？）~~ | **已解决 2026-07-29** | — |

### Q3 结论：剩余部分任意

对上所需对子后，剩下的牌位由玩家自由选择，不强制出最大。理由：

1. **策略深度** —— "垫哪张"是双升的核心决策之一，强制出最大等于剥夺该决策
2. **强制无收益** —— 首出甩牌已通过最大性校验（每个分量都是场上最大），
   跟牌方必然接不住，强制出最大只是白白损失大牌
3. **避免耦合** —— "必须出最大"需要跟牌方判断能否接住，会与甩牌最大性校验
   产生逻辑纠缠，复杂度不划算
