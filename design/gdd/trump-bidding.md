...
1. **发牌阶段**：庄家在发牌过程中可随时亮主声明（与首局界面机制一致）
2. **发牌结束**：如庄家已声明 → 进入配底阶段（C4）
3. **配底完成后**：开启反主窗口
...
| 参数 | 类型 | 默认值 | 影响 |
|------|------|--------|------|
| `bid_requires_joker` | bool | true | 是否需要王才能定主。false 时级牌单独可定。**需补充到 F3 参数表** |
...
| # | 问题 | 归属 | 优先级 |
|---|------|------|--------|
| Q1 | `bid_requires_joker` 需补充到 F3 规则配置系统的参数表中 | F3 更新 | 高 |
