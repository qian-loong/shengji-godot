...
> **Status**: Implemented (ADR-0001: 5-tier strength refinement; ADR-0002: JokerPairRank counter-immunity)
...
**全 5 档强度对照（`BidType` 枚举值即 strength）——见 [ADR-0001](../../docs/architecture/adr-0001-bid-strength-refinement.md) / [ADR-0002](../../docs/architecture/adr-0002-joker-pair-rank-counter-immunity.md)**：
...
9. **顶档免疫反主**（ADR-0002）：`PairJoker`（公主）与 `JokerPairRank`（王+对）均不可被反主——
   - `PairJoker` 因其为绝对最高强度（s=5）
   - `JokerPairRank`（s=4）作为"反公主屏障"：庄家投入 3 张关键牌（1 王 + 2 同花色级）凑齐"大亮"，
