...
> **Status**: Implemented (ADR-0001: 5-tier strength refinement; ADR-0002: JokerPairRank counter-immunity; ADR-0003: symmetric PairRank counter-immunity; ADR-0004: counter-bid uses current-round rank)
...
**全 5 档强度对照（`BidType` 枚举值即 strength）——见 [ADR-0001](../../docs/architecture/adr-0001-bid-strength-refinement.md) / [ADR-0002](../../docs/architecture/adr-0002-joker-pair-rank-counter-immunity.md) / [ADR-0003](../../docs/architecture/adr-0003-pair-rank-counter-immunity.md)**：

| Strength | BidType | 中文名 | 条件 | trump_suit | 适用模式 | 可被反主 |
|---:|---|---|---|---|---|---|
| 0 | NONE | — | 占位 | — | — | — |
| 1 | SingleRank | 单亮 | 1 张该花色级牌 | 该花色 | `bid_requires_joker = false` | ✅ |
| 2 | PairRank | 对亮 | 2 张该花色级牌（**严格同花色**） | 该花色 | `bid_requires_joker = false` | ❌（ADR-0003：与王+对对称免疫） |
| 3 | JokerSingleRank | 王+单 | 1 张王 + 1 张级牌（王色匹配如开启） | 级牌花色 | `bid_requires_joker = true` | ✅ |
| 4 | JokerPairRank | 王+对 | 1 张王 + 2 张该花色级牌（**严格同花色**；王色匹配如开启） | 级牌花色 | `bid_requires_joker = true` | ❌（ADR-0002：与公主同等免疫） |
| 5 | PairJoker | 公主 | 2 张同种王（双小王 / 双大王） | null（无主） | 任何模式 | ❌（最高档） |

**反主免疫总则**：**只有"单张"档（`SingleRank` / `JokerSingleRank`）可被反主；"对级"及以上（`PairRank` / `JokerPairRank` / `PairJoker`）一律免疫**。两种模式对称。
...
3. **条件**：反主声明的强度必须**严格大于**庄家当前的定主声明强度（同档不同花色不算更强，不可互反）。**同时**，反家可用的级牌部分必须是**当前局 rank**（`state.current_rank`，即庄家队级），而非反家自己队级——见 [ADR-0004](../../docs/architecture/adr-0004-counter-bid-uses-current-round-rank.md)。`PAIR_JOKER`（公主）用两张同种王，不消耗任何 rank 牌，豁免此约束
...
9. **对级及以上免疫反主**（ADR-0002 / ADR-0003）：只有"单张"档（`SingleRank` / `JokerSingleRank`）可被反主；`PairRank`、`JokerPairRank`、`PairJoker` 一律不可被反主，两种模式对称——
   - `PairJoker`（s=5）因其为绝对最高强度
   - `JokerPairRank`（s=4，ADR-0002）作为"反公主屏障"：庄家投入 3 张关键牌（1 王 + 2 同花色级）凑齐"大亮"，
     该投入应得到反主免疫的保障，避免被反家仅 2 张同种王的公主轻易翻盘
   - `PairRank`（s=2，ADR-0003）与 `JokerPairRank` 对称：模式 A 下投入"2 张同花色级牌"凑齐对亮，
     同属"对子投入"，不应被公主轻易翻盘。由此免疫规则在两种模式下完全一致：**对子投入即免疫**
...
| E12 | **反家的级牌 rank ≠ 当前局 rank** | 拒绝——错误 `counter_rank_mismatch`（ADR-0004 门 2）。跨 rank 场景下反家除了公主外只能用**当前局 rank** 的级牌 |
...
| E11 | **庄家亮对级（PairRank / JokerPairRank）被反** | 不可能——对级及以上免疫反主（ADR-0002 / ADR-0003），反主窗口不开启 |
...
| AC7 | 非首局，庄家亮 SingleRank，对手方亮 PairRank（**级牌 rank = 当前局 rank**）在反主窗口 | 反主成功，反家重新配底 |
| AC8 | 非首局，庄家亮 `PairRank`（对亮，模式 A） | 不可反主，反主窗口直接跳过（ADR-0003：与 `JokerPairRank` 对称免疫） |
| AC9 | 非首局，庄家亮 `PairJoker`（公主）或 `JokerPairRank`（王+对） | 不可反主，反主窗口直接跳过（ADR-0002） |
...
| AC19 | 庄家亮 `JokerPairRank ♥`（s=4），反家手握双大王 | 反主窗口不开启；即便强行 `submit_counter_or_pass` 也返回 `not_counter_window`（ADR-0002） |
| AC20 | 庄家亮 `PairRank ♥`（s=2，模式 A），反家手握双大王 | 反主窗口不开启；即便强行 `submit_counter_or_pass` 也返回 `not_counter_window`（ADR-0003） |
| AC21 | **两队级不同**（如南北队打 3、东西队打 2、庄家=北）；反家用**自己队级**的 `PairRank ♣`（rank=3）反庄家的 `SingleRank ♦`（rank=2） | **拒绝**——反家级牌 rank ≠ 当前局 rank（2），错误 `counter_rank_mismatch`（ADR-0004 门 2） |
| AC22 | 同 AC21 环境；反家用**当前局 rank** 的 `PairRank ♣`（rank=2）反庄家的 `SingleRank ♦`（rank=2） | 反主成功——strength 严格大于 且 rank 匹配（ADR-0004） |
| AC23 | 两队级不同环境下，反家用 `PairJoker`（公主）反庄家的 `SingleRank`（rank=2） | 反主成功——公主豁免 rank 约束（ADR-0004），且强度最高 |
...
| ~~Q2~~ | ~~非首局发牌阶段，非庄家玩家是否也可以亮主（抢庄）？~~ | **已解决 2026-07-28** | — |

### Q2 结论：可以，按轮转夺庄

本文 §3.3 与 §5.3 已经规定了答案，只是此前未与本表对齐：

> §3.3 轮到某人时：能定主 → 定主成功，**该人成为庄家**
> §5.3 庄家无法定主 → **下手敌方尝试定主当庄**，依次轮转

实现（`GameRound.process_bid` 无条件设 `dealer_seat = declaration.seat_id`）与之一致。
日志校验器 `validate_game_log.py` 的 `_check_dealer_rotation` 也按此判定：
庄家换人本身不算违规，但必须能由「原庄家起逐位 skip + 该座位 bid」的记录解释，
否则报 `dealer_rotation`。
