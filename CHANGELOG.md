...
### Sprint 3 进行中 (2026-07-01 至今)

**目标**: M1 图形 UI 表现层实现

**状态**: 14/18 任务完成 (78%)，核心可玩流程已就绪

#### 已完成 (Phase 1-2 + 部分 Phase 3-4)

- ✅ Kenney 卡牌资源集成 (68 张精美卡牌替代程序化占位符)
- ✅ Bootstrap 风格 SVG 花色图标 (统一 UI 视觉语言)
- ✅ 横屏牌桌布局 (1280×720 基准，四方座位 + 中央出牌区)
- ✅ `CardView` 组件 (支持选中、高亮、翻面)
- ✅ `HandDisplay` 手牌区 (水平排列 + 选牌上浮 + 多选)
- ✅ InfoBar 顶部信息栏 (级、主、庄家、得分实时显示)
- ✅ 亮主选择器浮层 (图形化花色选择 + 强度指示)
- ✅ 反主窗口 UI (完整流程替代 TUI 短路)
- ✅ 配底交互 (选牌扣底 + 视觉反馈)
- ✅ 出牌确认 + 非法提示
- ✅ 结算界面 (分步展示 + 底牌翻开)
- ✅ 主菜单场景 (GUI/TUI 模式切换 + 设置占位符)
- ✅ Android 构建支持 (APK 导出脚本 + 模拟器 QA 验证)
...
- ⏭️ 赢墩收回动画
- ⏭️ 游戏结束画面
- ⏭️ 音频系统 (BGM + SFX，详见 `design/gdd/audio-system.md`)

#### 待完成

- ⬜ 文档更新 (CHANGELOG + systems-index 验证)
