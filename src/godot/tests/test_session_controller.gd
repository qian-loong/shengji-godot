...
func test_counter_window_skipped_on_joker_pair_rank_bid() -> void:
	# ADR-0002: dealer plays JOKER_PAIR_RANK (王+对) — also immune to counter.
	# Counter window must NOT open; controller advances directly to "playing".
	_setup_at_burying(false, 0, TrumpBidding.BidType.JOKER_PAIR_RANK, Card.Suit.HEART)
	_dealer_burys_and_advance()
