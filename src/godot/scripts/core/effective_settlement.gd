...
##     不知道跨局约束（必打级等）。视作只读值对象。
##   - EffectiveSettlement 是"会话层的最终裁决"，在提案基础上叠加
##     跨局规则（必打级已由 UpgradeSettlement 处理），字段与 state.team_ranks
##     等真实状态严格一致。
##
...
var upgrade_levels: int          # 生效等级数
var new_rank: int                # 生效后的升级结果
...
static func from_proposal(
	...
	e.upgrade_levels = proposal.upgrade_levels
